...
def test_token_expired(
    ...
    assert token_expired("url", 18000) is True

    mock_load_env.assert_called_once_with(
        dotenv_path="src/.env",
        override=True,
    )
    mock_open_auth_url.assert_called_once_with("url", 18000)
...
def test_expiry_none(mock_load_env: MagicMock, monkeypatch):
    ...
    assert token_expired("url", 18000) is False
